  ...
  try {
    ...
    async function padBurstQueueToLimit(items, targetLimit) {
      ...
      while (out.length < target && scanned < maxPadScan && padTypeIndex < activeTypes.length) {
        ...
        for (const cat of page) {
          ...
          if (shouldSkipCatalogIngest(cat)) continue;
    ...
    while (!stopWindow && chunkDone < LIMIT && Date.now() < deadlineMs) {
      ...
      if (source === "cursor") {
        ...
        if (cat.nonCanonicalGear || isNonCanonicalGearVariant(cat)) {
          skipped += 1;
          processed += 1;
          console.log(`SKIP ${setNo} (non-canonical GEAR *-N — ${cat.catalogItemId})`);
